...
df=pandas.read_csv("/Users/shilpibouddha/PycharmProjects/app2-portfolio/StudentProjectDay22/data (2).csv")

with col1:
    for index, row in df[:4].iterrows():
        st.subheader(f"{row["first name"].title()} {row["last name"].title()}")
        st.write(row["role"])
        st.image("/Users/shilpibouddha/PycharmProjects/app2-portfolio/StudentProjectDay22/images (2)/"+ row["image"])
